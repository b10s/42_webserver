...
class HttpRequestAdvanceBody : public ::testing::Test {
 protected:
  HttpRequest req;
};
...
TEST_F(HttpRequestAdvanceBody, AdvanceBody_ContentLength_HappyPath) {
...
TEST_F(HttpRequestAdvanceBody, AdvanceBody_ContentLength_SizeThenData) {
...
TEST_F(HttpRequestAdvanceBody, AdvanceBody_ContentLength_LeavesExtraDataForNextRequest) {
...
TEST_F(HttpRequestAdvanceBody, AdvanceBody_Chunked_HappyPath) {
...
TEST_F(HttpRequestAdvanceBody, AdvanceBody_Chunked_MultipleRecvs) {
...
// =============== Need more data ===============
TEST_F(HttpRequestAdvanceBody, AdvanceBody_ContentLength_NeedMoreData) {
  req.SetBufferForTest("Hello");
  req.SetContentLengthForTest(12);
...
// =============== Malformed requests ===============
TEST_F(HttpRequestAdvanceBody, AdvanceBody_Chunked_Malformed_ChunkSize) {
  req.SetBufferForTest("G\r\nhello\r\n"); // invalid hex 'G'
  req.SetContentLengthForTest(-1); // chunked

  EXPECT_THROW(req.AdvanceBody(), lib::exception::ResponseStatusException);
}

TEST_F(HttpRequestAdvanceBody, AdvanceBody_Chunked_Malformed_NoFinalCRLF) {
...
TEST_F(HttpRequestAdvanceBody, AdvanceBody_Chunked_ChunkSizeWithExtensions) {
...
// =============== Edge cases ===============
TEST_F(HttpRequestAdvanceBody, AdvanceBody_ContentLength_ZeroLength) {
...
// extra data after "0\r\n\r\n" -> 400 Bad Request
TEST_F(HttpRequestAdvanceBody, AdvanceBody_Chunked_ExtraDataAfterTerminator_ThrowsBadRequest) {
  req.SetBufferForTest("5\r\nhello\r\n0\r\n\r\nGARBAGE");
  req.SetContentLengthForTest(-1);
...
TEST_F(HttpRequestAdvanceBody, AdvanceBody_Chunked_ChunkSizeOverflow_ThrowsBadRequest) {
  req.SetContentLengthForTest(-1);
...
TEST_F(HttpRequestAdvanceBody, AdvanceBody_Chunked_ChunkSizeLargeButValid_WorksCorrectly) {
  req.SetContentLengthForTest(-1);
  ...
  req.SetBufferForTest(payload);
  bool done = false;
  EXPECT_NO_THROW(done = req.AdvanceBody());
