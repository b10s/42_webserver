...
static void expectErrorPageEq(const ServerConfig& sc,
                              int code,
...
static void callParseErrorPage(const std::string& input,
                               ServerConfig* sc) {
    ConfigParser parser;
...
TEST(ConfigParser, ErrorPage_MissingSemicolon_Throws) {
    ServerConfig sc;
    EXPECT_THROW(callParseErrorPage("404 /errors/404.html", &sc), std::runtime_error);
}