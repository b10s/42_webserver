...
TEST(FileValidaterTest, ValidateAndNormalizePath_BasicCases) {
  ...
  EXPECT_NO_THROW(FileValidater::ValidateAndNormalizePath(path1, document_root1));
  EXPECT_EQ(FileValidater::ValidateAndNormalizePath(path1, document_root1), path1);
}
...
TEST(FileValidaterTest, ValidateAndNormalizePath_WithDotDotSegments) {
  ...
  EXPECT_THROW(FileValidater::ValidateAndNormalizePath(path2, document_root2), lib::exception::ResponseStatusException);
}
...
TEST(FileValidaterTest, ValidateAndNormalizePath_WithUnsafeChars) {
  ...
  EXPECT_THROW(FileValidater::ValidateAndNormalizePath(path3, document_root3), lib::exception::ResponseStatusException);
}
...
TEST(FileValidaterTest, ValidateAndNormalizePath_NotUnderDocumentRoot) {
  ...
  EXPECT_THROW(FileValidater::ValidateAndNormalizePath(path4, document_root4), lib::exception::ResponseStatusException);
}
...
TEST(FileValidaterTest, ValidateAndNormalizePath_Normalization) {
  ...
  EXPECT_NO_THROW(FileValidater::ValidateAndNormalizePath(path5, document_root5));
  EXPECT_EQ(FileValidater::ValidateAndNormalizePath(path5, document_root5), expected_normalized_path5);
}