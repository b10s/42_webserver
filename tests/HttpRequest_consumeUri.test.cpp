...
TEST_F(HttpRequestParseUri, UriTooLong_Throws414_WhenReachesLimitExactly) {
  ...
  std::string path =
      "/" + makeN('a', N);  // '/' + N 個 = N+1 文字読む → 途中で throw
...
//   std::string acceptable = "/" + makeN('a', N - 1);  // 読む文字数 = N
//   （'/'含む）→ 直前でOK std::string s = acceptable + " HTTP/1.1";
  ...
  EXPECT_THROW(
      {
        try {
          ...
          EXPECT_EQ(BAD_REQUEST,
                    e.getStatus());  // 末尾'&'で次キー空 → 方針で変えてOK
          throw;
        }
      },
      http::responseStatusException);
